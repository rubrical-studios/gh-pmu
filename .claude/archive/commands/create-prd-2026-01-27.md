<!-- Archived: 2026-01-27T23:24:25.972Z -->
<!-- Original: E:\Projects\gh-pmu\.claude\commands\create-prd.md -->
...
|------|------------|-------------|
| **Issue-Driven** | `/create-prd 123` or `/create-prd #123` | Transform proposal (from issue) to PRD |
...
Parse issue body for proposal document reference:
```
...
|-------|-------|-----------|
| Unit | Individual functions/components | [from Inception/Tech-Stack.md] |
| Integration | Cross-component flows | [from Inception/Tech-Stack.md] |
| E2E | Critical user journeys | TBD |
...
|------|--------|-----------|
| Unit | 80%+ | Core logic |
| Integration | Key flows | Boundary verification |
| E2E | Critical paths | Journey validation |
...
**Update test plan with issue number:**

After issue creation, update the Test Plan frontmatter with the approval issue number.
...
Get existing `projectSkills` array (may be empty or absent).

### Step 2: Extract Technical Requirements

Parse the PRD for technical indicators:
- **Tech Requirements section** - specific technologies mentioned
- **User Stories** - technical terms in acceptance criteria
- **Non-Functional Requirements** - CI/CD, security, performance mentions

### Step 3: Match Against Skill Triggers

Compare PRD content against skill registry triggers:

| PRD Content | Matched Skill |
|-------------|---------------|
| "CI/CD pipeline" mentioned | `ci-cd-pipeline-design` |
| "E2E tests" in requirements | `playwright-setup` |
| "API versioning" needed | `api-versioning` |

### Step 4: Identify New Skills

Filter out skills already in `projectSkills`:
