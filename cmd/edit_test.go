...
// ============================================================================
// IDPF Claude Reminder Tests
// ============================================================================

func TestRunEditWithDeps_IDPFProjectAddsClaudeReminder(t *testing.T) {
	mock := setupMockForEdit()
	cfg := &config.Config{
	...
	if len(mock.updateBodyCalls) != 1 {
		t.Fatalf("Expected 1 body update call, got: %d", len(mock.updateBodyCalls))
	}
	if !contains(mock.updateBodyCalls[0], "Claude:") {
		t.Error("Expected Claude reminder to be prepended for IDPF project")
	}
	if !contains(mock.updateBodyCalls[0], "## Problem") {
		t.Error("Expected original body content to be preserved")
	}
}

func TestRunEditWithDeps_NonIDPFProjectNoClaudeReminder(t *testing.T) {
	mock := setupMockForEdit()
	cfg := &config.Config{
		Project:      config.Project{Owner: "testowner", Number: 1},
		Repositories: []string{"testowner/testrepo"},
...
func TestRunEditWithDeps_IDPFProjectDoesNotDuplicateReminder(t *testing.T) {
	mock := setupMockForEdit()
	cfg := &config.Config{
		Project:      config.Project{Owner: "testowner", Number: 1},
		Repositories: []string{"testowner/testrepo"},
	...
	cmd, _ := newTestEditCmd()
	// Body already contains a Claude reminder
	opts := &editOptions{
		issueNumber: 123,
	...
	// Count "Claude:" occurrences - should be exactly 1
	count := 0
	for i := 0; i <= len(mock.updateBodyCalls[0])-7; i++ {
		if mock.updateBodyCalls[0][i:i+7] == "Claude:" {
			count++
		}
	}
	if count != 1 {
